// GridCellButton.cpp : implementation file
//
...
// 13 Mar 2004 - GetCellExtent and GetButtonPlacement fixed by Yogurt
...
CGridCellButton::CGridCellButton() : CGridCell()
{
    m_bButtoned = FALSE;
    //m_Rect.IsRectNull();
}
...
BOOL CGridCellButton::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    pDC->DrawFrameControl(GetButtonPlacement(), DFC_BUTTON, 
	    DFCS_BUTTONPUSH|DFCS_BUTTON3STATE);
...
void CGridCellButton::OnClick(CPoint PointCellRelative)
{
	...
	// GetButtonPlacement returns the checkbox dimensions in client coords. Only check/
	// uncheck if the user clicked in the box
	if (GetButtonPlacement().PtInRect(PointCellRelative))
	{
		m_bButtoned = !m_bButtoned;
...
BOOL CGridCellButton::SetButton(BOOL bButtoned /*=TRUE*/)
{
	BOOL bTemp = m_bButtoned;
	m_bButtoned = bButtoned;
...
BOOL CGridCellButton::GetButton()
{
	return m_bButtoned;
}
