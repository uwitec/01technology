...
{	
	...
	lstrcpyn(m_GameServiceAttrib.szDescription,TEXT("拱猪GameService组件"),CountArray(m_GameServiceAttrib.szDescription));
	lstrcpyn(m_GameServiceAttrib.szServerModuleName,TEXT("B_UncoverPig.DLL"),CountArray(m_GameServiceAttrib.szServerModuleName));
	m_GameServiceAttrib.wProcessType = ID_B_UNCOVERPIG;
...
bool __cdecl CGameServiceManager::RectifyServiceOption(tagGameServiceOption * pGameServiceOption)
{
	...
	//额度调整
	pGameServiceOption->dwCellScore=__max(2L,pGameServiceOption->dwCellScore);
