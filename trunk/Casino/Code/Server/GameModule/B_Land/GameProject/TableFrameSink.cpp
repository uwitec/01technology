	...
	case GER_NORMAL:		//常规结束
		{
			...
			for (WORD i=0;i<m_wPlayerCount;i++)
			{
				if (i==m_wBankerUser) GameEnd.fGameScore[i]=m_wBombTime*m_bLandScore*fCellScore*((bLandWin==true)?2:-2);
				else GameEnd.fGameScore[i]=m_wBombTime*m_bLandScore*fCellScore*((bLandWin==true)?-1:1);
			}
...
bool CTableFrameSink::OnUserLandScore(WORD wChairID, BYTE bLandScore)
{
	...
	if ((m_bLandScore==3)||(m_wFirstUser==(wChairID+1)%m_wPlayerCount))
	{
		...
		//投注
		tagBetScoreInfo BetScoreInfo;
		BetScoreInfo.dwBetScore = m_pGameServiceOption->dwCellScore * m_bLandScore;
		BetScoreInfo.wBetRegionIndex = 0;
		for(int i = 0; i < m_pGameServiceAttrib->wChairCount; i ++)
		{
			IServerUserItem * pIServerUserItem = m_pITableFrame->GetServerUserItem(i);
