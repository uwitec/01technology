package 
{
	...
	public class GameView extends GameBaseView
	{
		...
		public var m_pScoreView:Array =  new Array(GAME_PLAYER);//得分视图
		...
		public var m_bDoubleTime:Array = new Array(GAME_PLAYER);//用户反牌
		public var m_bDoubleTimeView:Array = new Array(GAME_PLAYER);//用户反牌
		public var m_BreakRuleView:BreakRuleView;//不符合规则
		...
		public var m_btOneScore:ImageButton;//1分按钮
		public var m_btTwoScore:ImageButton;//2分按钮
		...
		public var m_btDoubleTime:ImageButton;//反牌按钮
		public var m_btGiveUpDoubleTime:ImageButton;//不反按钮
		...
		public var m_WarnRemainOneCardView:Array=new Array(3);//报牌一张
		public var m_WarnRemainTwoCardView:Array=new Array(3);//报牌二帐
		...
		override public function InitGameView():Boolean
		{
			...
			m_btDoubleTime=new ImageButton(IMAGE_BT_DoubleTime,425,37);
			m_btDoubleTime.addEventListener(MouseEvent.CLICK,btnDoubleTime);
			addChild(m_btDoubleTime);
			m_btGiveUpDoubleTime=new ImageButton(IMAGE_BT_GiveUpDoubleTime,425,37);
			m_btGiveUpDoubleTime.addEventListener(MouseEvent.CLICK,btnGiveUpDoubleTime);
			addChild(m_btGiveUpDoubleTime);
			...
			m_btMoreTime.addEventListener(MouseEvent.CLICK,OnEventMoreTme);
			...
			m_btDoubleTime.ShowWindow(false);
			m_btGiveUpDoubleTime.ShowWindow(false);
			...
			for(i=0;i<GAME_PLAYER;i++)
			{
				m_WarnRemainOneCardView[i] = new WarnRemainOneCardView;
				addChild(m_WarnRemainOneCardView[i]);
				m_WarnRemainTwoCardView[i] = new WarnRemainTwoCardView;
				addChild(m_WarnRemainTwoCardView[i]);
			}
			
			m_BreakRuleView = new BreakRuleView;
			addChild(m_BreakRuleView);
		...
		override public function DestroyGameView():void
		{
			if(m_bFreeChildClassViewData == false)
			{
				...
				m_ptScore = null;
				ZeroArray(m_bDoubleTimeView,null, removeChild);
				m_bDoubleTimeView = null;
				...
				m_btDoubleTime.Destroy();
				m_btDoubleTime.removeEventListener(MouseEvent.CLICK,btnDoubleTime);
				removeChild(m_btDoubleTime);
				m_btDoubleTime = null;
				m_btGiveUpDoubleTime.Destroy();
				m_btGiveUpDoubleTime.removeEventListener(MouseEvent.CLICK,btnGiveUpDoubleTime);
				removeChild(m_btGiveUpDoubleTime);
				m_btGiveUpDoubleTime = null;
				...
				m_btMoreTime.removeEventListener(MouseEvent.CLICK,OnEventMoreTme);
				...
				{
					removeChild(m_WarnRemainOneCardView[i]);
					m_WarnRemainOneCardView[i] = null;
					removeChild(m_WarnRemainTwoCardView[i]);
					m_WarnRemainTwoCardView[i] = null;
				}
				m_WarnRemainOneCardView = null;
				m_WarnRemainTwoCardView = null;
				
				removeChild(m_BreakRuleView);
				m_BreakRuleView = null;
		...
		override protected function  DrawGameView(g:Graphics,pDC:BitmapData):void
		{
			...
			var strChairID:Array = ["①","②","③"];
			var n:int = 0
			for(var i:uint = 0; i < m_bDoubleTime.length; i ++)
			{
				if(m_bDoubleTime[i] != 0)
				{
					var pUserData:tagUserInfoHead=GetUserInfo(i);
					if(pUserData == null || pUserData.UserStatus.wChairID == INVALID_CHAIR)
						continue;
					DrawUtil.TextOut(pDC,
					 218,
					 46 + n * 18,
					 strChairID[pUserData.UserStatus.wChairID] + "座 " + 
					  m_GameClass.GetServerAttribute().fCellScore.toFixed(0) +
					  " X " +
					  m_bDoubleTime[i] ,
				 	 18,
		...
		override public function RectifyGameView( nWidth:int, nHeight:int):void
		{
			...
			m_btOneScore.MoveWindow(nWidth/2-45-rcButton.width/2,548);
			m_btGiveUpScore.MoveWindow(nWidth/2+45-rcButton.width/2,548);
			m_btDoubleTime.MoveWindow(nWidth/2-45-rcButton.width/2,548);
			m_btGiveUpDoubleTime.MoveWindow(nWidth/2+45-rcButton.width/2,548);
			...
			m_WarnRemainOneCardView[0].x = m_ptScore[0].x+ 70;
			m_WarnRemainOneCardView[0].y = m_ptScore[0].y+ 40;
			m_WarnRemainTwoCardView[0].x = m_ptScore[0].x+ 70;
			m_WarnRemainTwoCardView[0].y = m_ptScore[0].y+ 40;
	
			m_WarnRemainOneCardView[1].x = m_ptScore[1].x + 70;
			m_WarnRemainOneCardView[1].y = m_ptScore[1].y + 50;
			m_WarnRemainTwoCardView[1].x = m_ptScore[1].x + 70;
			m_WarnRemainTwoCardView[1].y = m_ptScore[1].y + 50;

			m_WarnRemainOneCardView[2].x = m_ptScore[2].x+ 50;
			m_WarnRemainOneCardView[2].y = m_ptScore[2].y+ 40;
			m_WarnRemainTwoCardView[2].x = m_ptScore[2].x+ 50;
			m_WarnRemainTwoCardView[2].y = m_ptScore[2].y+ 40;
			
			m_BreakRuleView.x = 660;
			m_BreakRuleView.y = 520;
		...
		private function OnEventMoreTme(e:MouseEvent):void
		{
			dispatchEvent(new GameViewEvent(GameViewEvent.nGVM_MORE_TIME,0,0));
		}
		private function OnEventLeftHitCard(e:GameViewEvent):void
		{
			dispatchEvent(e.cloneEvent());
		}
		private function btnDoubleTime(e:MouseEvent):void
		{
			dispatchEvent(new GameViewEvent(GameViewEvent.nGVM_DOUBLETIME,0,0));
		}
		private function btnGiveUpDoubleTime(e:MouseEvent):void
		{
			dispatchEvent(new GameViewEvent(GameViewEvent.nGVM_GIVEUPDOUBLETIME,0,0));
		}
		...
		//报牌警告
		public function WarnRemainCard(wChairID:uint,cbCardCount:uint):void
		{
			if(cbCardCount == 1)
			{
				m_WarnRemainOneCardView[wChairID].gotoAndPlay(2);
			}
			else if(cbCardCount == 2)
			{
				m_WarnRemainTwoCardView[wChairID].gotoAndPlay(2);
			}
		}
		//设置放弃
		...
		//反牌
		public function SetUserDoubleTime( wChairID:uint,  bDoubleTime:uint):void
		{
			//设置变量
			if (wChairID!=INVALID_CHAIR)
			{
				m_bDoubleTime[wChairID]=bDoubleTime;
			}
			else
			{
				for(var i:uint = 0; i < m_bDoubleTime.length; i ++)
		...
		//反牌
		public function SetUserDoubleView( wChairID:uint,  bDoubleTime:uint):void
		{
			//设置变量
			if (wChairID!=INVALID_CHAIR)
			{
				UpdateUserDoubleView(wChairID,bDoubleTime);
			}
			else
			{
				for(var i:uint = 0; i < m_bDoubleTime.length; i ++)
				{
					UpdateUserDoubleView(i,0);
				}
		...
		//不符合规则
		public function ShowBreakRuleView(b:Boolean):void
		{
			if(m_BreakRuleView && b)
			{
				m_BreakRuleView.stop();
				m_BreakRuleView.gotoAndPlay(2);
			}
		}
		//设置上轮
		...
		protected function UpdateUserDoubleView(wChairID:uint,  bDoubleTime:uint):void
		{
			if(bDoubleTime != 0)
			{
				if(m_bDoubleTimeView[wChairID] == null)
				{
					var xp:Number  = m_ptScore[wChairID].x;
					var yp:Number  = m_ptScore[wChairID].y;
					switch(wChairID)
					...
					{
						case 1:
							m_bDoubleTimeView[wChairID] = new GiveUpDoubleTimeView;
						break;
						case 2:
							m_bDoubleTimeView[wChairID] = new DoubleTimeView;
						break;
					}
					
					if(m_bDoubleTimeView[wChairID] != null)
					{
						addChild(m_bDoubleTimeView[wChairID]);
						m_bDoubleTimeView[wChairID].x = xp;
						m_bDoubleTimeView[wChairID].y = yp;
					}
				}
			}
			else
			{
				if(m_bDoubleTimeView[wChairID])
				{
					removeChild(m_bDoubleTimeView[wChairID]);
					m_bDoubleTimeView[wChairID] = null;
				}
			}
		}
