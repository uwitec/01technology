package 
{
	...
	import org.god.Common.CaptureRgnEvent;
	import org.god.Common.CaptureRgnHelper;
	...
	{
		//数值掩码
		protected const CARD_MASK_COLOR:uint=0xF0;//花色掩码
		protected const CARD_MASK_VALUE:uint=0x0F;//数值掩码
		//配置变量
		...
		protected var m_CaptureRgnHelper:CaptureRgnHelper;//选择区域
		...
		{
			if(m_CaptureRgnHelper != null)
			{
				m_CaptureRgnHelper.removeEventListener(CaptureRgnEvent.EVENT_NAME,
														OnEventCaptureRgn);
				m_CaptureRgnHelper.Destroy();
				m_CaptureRgnHelper = null;
			}
		...
		public function SetPositively( bPositively:Boolean):void
		{
			...
			if(m_bPositively)
			{
				if(m_CaptureRgnHelper == null)
				{
					m_CaptureRgnHelper = new CaptureRgnHelper(this);
					m_CaptureRgnHelper.Create();
					m_CaptureRgnHelper.addEventListener(CaptureRgnEvent.EVENT_NAME,
														OnEventCaptureRgn);
				}
			}
			else
			...
			if (m_bHorizontal==true)
			{
				...
				if (bCurrentCard==false)
				{
					...
					while (dwCardIndex>0)
					{
						...
						/*if ((bCardShoot==false)&&(MousePoint.y>m_CardSize.y))
						var nRightPos:uint = dwCardIndex * m_dwCardHSpace + dwDetalSpace;
						if ((bCardShoot==false)&&(MousePoint.y>m_dwShootAltitude) && 
							MousePoint.x <= nRightPos)
						{
		...
		public function OnPaint(pDC:BitmapData,rcClient:Rectangle):void
		{
			...
			for (var i:uint=0;i<m_CardDataItem.length;i++)
			{
				...
				{
					dwXImagePos=((pCardItem.bCardData&CARD_MASK_VALUE)-1)*m_CardSize.x;
					dwYImagePos=((pCardItem.bCardData&CARD_MASK_COLOR)>>4)*m_CardSize.y;
				}
				else
				{
		...
		public function OnEventMouseUp(e:MouseEvent):void
		{
			...
			//发送消息
			dispatchEvent(new GameViewEvent(GameViewEvent.nGVM_LEFT_HIT_CARD));
			//调整控件
			RectifyControl();
	   ...
	   public function OnEventMouseMove(e:MouseEvent):void
		...
		public function OnEventCaptureRgn(e:CaptureRgnEvent):void
		{
			var pointBegin:Point = new Point(e.m_XPos,e.m_YPos);
			var pointEnd:Point = new Point(e.m_XPos + e.m_Width,e.m_YPos + e.m_Height);
			
			var dwCurrentIndexBegin:uint=SwitchCardPoint(pointBegin);
			var dwCurrentIndexEnd:uint=SwitchCardPoint(pointEnd);
			var dwCardCount:uint=m_CardDataItem.length;
			if (dwCurrentIndexBegin < dwCardCount && dwCurrentIndexEnd < dwCardCount)
			{
				for(var i:int = dwCurrentIndexBegin; i <= dwCurrentIndexEnd; i ++)
				{
					//设置控件
					var  pCardItem:tagCardItem=m_CardDataItem[i];
					pCardItem.bShoot=!pCardItem.bShoot;
