package 
{
	...
	public class LogonWindow extends MovieClip implements ILogonWindow
	{
		...
		public function LogonWindow(mcParent:IMain)
		{
			...
			addChild(m_tfPasswd);
			m_tfPasswd.setComBoundsXYWH(nXPos, nYPos+30, nWidth, nHeight);
			
			m_tfSecurity= new SkinTextField();
			addChild(m_tfSecurity);
			m_tfSecurity.setComBoundsXYWH(nXPos, nYPos+60, nWidth/2+10, nHeight);
			
			m_labelSecurity= new JLabel("");
			addChild(m_labelSecurity);
			m_labelSecurity.setComBoundsXYWH(nXPos+nWidth/2-5, nYPos+60, nWidth, nHeight);
			var MyFont:ASFont = new ASFont;
			MyFont = MyFont.changeSize(15);
			MyFont = MyFont.changeBold(true);
			m_labelSecurity.setFont(MyFont);
			var MyFontColor:ASColor = new ASColor(0xe11010);
			...
			m_cbxLogonStation.setComBoundsXYWH(nXPos, nYPos +35 + 30 + 25, nWidth, nHeight+2);

			m_btnLogonWindow = new SkinButton(IDS_LOGON, nXPos+5, nYPos + 86+35 );
			...
			m_timerCheckLogonWindow = new Timer(10000, 1);
		...
		private function OnEventLogon(e:MouseEvent)
		{
			...
			m_IMain.getClientSocket().Connect(strLogonStationAddress, nPlazaServerPort);
		...
		{			
			...
			CmdData.cbClientType = CLIENTTYPE_MAN;
