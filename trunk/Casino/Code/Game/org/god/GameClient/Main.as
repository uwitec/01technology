package 
{
	...
	{
		public static const nStageWidth:int = 1024;//舞台宽度
		public static const nStageHeight:int = 768;//舞台高度
		...
		public static const nSWFModuleIndex_Panel:int = SWFModuleArray.indexOf("GamePane");
		public static const nSWFModuleIndex_Panel_Common:int = SWFModuleArray.indexOf("PlazaWindow_Common");

		private var m_ILogonWindow:ILogonWindow;//登陆窗口接口
		private var m_IPlazaWindow:IPlazaWindow;//大厅窗口接口
		private var m_IRuleWindow:IRuleWindow;//协议窗口接口
		...
		private var m_wUserType:uint = 0;//用户类型
					...
					case nMainViewType_LogonWindow :
						{
							...
							if(m_IRuleWindow != null)
							{
								m_IRuleWindow.Hide();
							}
						}
						break;
					case nMainViewType_RuleWindow :
						{
				...
				case nMainViewType_RuleWindow :
					{
						if(m_IRuleWindow != null)
						{
							return m_IRuleWindow.getActiveDOC();
						}
					}
				default :
					return this;
		...
		//设置用户类型
		public function setUserType(wUserType:uint):void
		{
			m_wUserType = wUserType;
		}
		//获取用户类型
		...
		//设置游戏信息
		public function setGameInfo(GameInfo:CMD_GP_GameInfo):void
		{
			m_GameInfo = GameInfo;
		}
		//获取游戏信息
		public function getGameInfo():CMD_GP_GameInfo
		{
			return m_GameInfo;
		}
		//设置公告
		public function setNotice(Notice:CMD_GP_Notice):void
		{
			m_Notice = Notice;
		}
		//获取公告
		public function getNotice():CMD_GP_Notice
		{
			return m_Notice;
		}
		//////////////////////
		...
		private function OnEventFullScreen(e:FullScreenEvent):void
		{
			...
			else
			{
				...
				if(m_IPlazaWindow)
				{
					m_IPlazaWindow.OnEventToggleFullScreen(false);
				}
			}
		}
		//////////////////////////////////////////////////
		...
		public function GetAppDomain(strModuleName:String):ApplicationDomain
		{
			...
			{
				if(strModuleName == "GamePane")
				{
					module = m_SWFModule.get("PlazaWindow");
					if(module != null)
						return module.getApplicationDomain();
				}
				...
				{
					var strTmp:String;
					if(strModuleName == "BetHistoryPanel_Common")
					   strTmp = "PlazaWindow";
					else
						strTmp = strModuleName.substring(0, n);
		...
		{
			m_LoaderPane = new WindowLoaderPane(this,2);
		...
		public function OnEventWindowLoadAllComplete(e:LoaderTaskAllCompleteEvent):void
		{
			if(m_ILogonWindow == null)
			{
				...
				c = getDefClass("RuleWindow","RuleWindow");
				if(c != null)
				{
					m_IRuleWindow = new c(this);
				}
			}
			if(m_ILogonWindow != null)
			{
				addChildDO(m_ILogonWindow.getActiveDOC(),false);
				addChildDO(m_IRuleWindow.getActiveDOC(),false);
