...
LRESULT CGridCtrl::SendMessageToParent(int nRow, int nCol, int nMessage) const
{
    ...
    CWnd *pOwner = GetMyOwner();
    if (pOwner && IsWindow(pOwner->m_hWnd))
        return pOwner->SendMessage(WM_NOTIFY, nmgv.hdr.idFrom, (LPARAM)&nmgv);
    else
        return 0;
}
CWnd* CGridCtrl::GetMyOwner () const
{
	return(CWnd*) this->GetParent();
}
                          ...
                          LPARAM data /* = 0 */)
{
    ...
    ResetSelectedRange();
    SetFocusCell(-1, -1);
    return SortItems(pfnCompare, nCol, bAscending, data, GetFixedRowCount(), GetDefaultSortHighRow());
}
...
int CALLBACK CGridCtrl::pfnCellNumericCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	double nValue1 = _tstof(pCell1->GetText());
	double nValue2 = _tstof(pCell2->GetText());
...
BOOL CGridCtrl::IsCellEditable(CCellID &cell) 
{
    return IsCellEditable(cell.row, cell.col);
}

BOOL CGridCtrl::IsCellEditable(int nRow, int nCol) 
{
...
BOOL CGridCtrl::SetWindowPos(const CWnd* pWndInsertAfter, int x, int y,
...
void  CGridCtrl::MoveWindow(int x, int y, int nWidth, int nHeight,
				BOOL bRepaint)
{

		CWnd::MoveWindow(x, y, nWidth, nHeight, bRepaint);
}
void  CGridCtrl::MoveWindow(LPCRECT lpRect, BOOL bRepaint)
{
