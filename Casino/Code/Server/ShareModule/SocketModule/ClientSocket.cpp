...
bool __cdecl CClientSocket::SetSocketSink(IUnknownEx * pIUnknownEx)
{
	...
	GT_ASSERT(m_pIClientSocketSink!=NULL);
...
bool __cdecl CClientSocket::Connect(DWORD dwServerIP, WORD wPort)
{
	...
	GT_ASSERT(m_SocketState==SocketState_NoConnect);
...
{
	//效验参数
	GT_ASSERT(m_dwSendPacketCount>0);
	GT_ASSERT(wDataSize>=sizeof(CMD_Head));
...
LRESULT CClientSocket::OnSocketNotifyRead(WPARAM wParam, LPARAM lParam)
{
	try
	{
		...
		while (m_wRecvSize>=sizeof(CMD_Head))
		{
			...
			GT_ASSERT(pHead->CmdInfo.cbMessageVer==SOCKET_VER);
			GT_ASSERT(wPacketSize<=(SOCKET_PACKAGE+sizeof(CMD_Head)));
