...
//判断是否双鬼
bool CGameLogic::HaveGuiPairCard(const BYTE bCardData[], BYTE bCardCount)
{
	bool bFlag[2];bFlag[0] = bFlag[1] = false;
	for(BYTE i = 0; i < bCardCount; i ++)
	{
		if(bCardData[i] == 0x41)
			bFlag[0] = true;
	...
	return bFlag[0] & bFlag[1];
}
//判断是否4条2
bool CGameLogic::HaveFour2Card(const BYTE bCardData[], BYTE bCardCount)
{
	bool bFlag[4];bFlag[0] = bFlag[1] = bFlag[2] = bFlag[3] = false;
	for(BYTE i = 0; i < bCardCount; i ++)
	{
		if(bCardData[i] == 0x02)
			bFlag[0] = true;
	...
	return bFlag[0] & bFlag[1] & bFlag[2] & bFlag[3];
}
//////////////////////////////////////////////////////////////////////////
