...
public:
	...
	bool CreateInstance()
	{
		...
		try
		{
			...
			if (m_hDllInstance==NULL) 
			{
				_sntprintf(m_szErrorMsg,sizeof(m_szErrorMsg),TEXT("%s 加载失败，错误代码：%ld ！"),m_pszModuleDll,GetLastError());
				return false;
			}
			...
			if (CreateFunc==NULL) 
			{
				_sntprintf(m_szErrorMsg,sizeof(m_szErrorMsg),TEXT("%s 组件创建函数 %s 不存在，错误代码：%ld ！"),m_pszModuleDll,m_pszFuncName,GetLastError());
				return false;
			}
			...
			{
				_sntprintf(m_szErrorMsg,sizeof(m_szErrorMsg),TEXT("%s 组件创建函数 %s 创建组件失败 ！"),m_pszModuleDll,m_pszFuncName);
				return false;
			}
		}
		catch (...)	
		{ 
			_sntprintf(m_szErrorMsg,sizeof(m_szErrorMsg),TEXT("%s 组件创建函数 %s 发生了异常错误 ！"),m_pszModuleDll,m_pszFuncName);
			return false;
		}
...
public:
	//指针重载
	IModeluInterface * operator->() { ASSERT(IsValid()); return GetInterface(); }
	//获取接口
