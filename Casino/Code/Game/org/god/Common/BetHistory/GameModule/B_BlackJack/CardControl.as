package org.god.Common.BetHistory.GameModule.B_BlackJack
{
	...
	public class CardControl extends MovieClip
	{
		...
		protected  var CARD_WIDTH:Number=70;//扑克宽度
		protected  var CARD_HEIGHT:Number=95;//扑克高度
		...
		//静态变量
		protected  var m_ImageCard:SkinImage;//扑克图片
		protected  var m_ImageBack:SkinImage;//扑克图片
		...
		public function CardControl(pImageCard:SkinImage)
		{
			...
			m_ImageBack = pImageCard;
			CARD_WIDTH=m_ImageCard.GetWidth()/13.0;
			CARD_HEIGHT=m_ImageCard.GetHeight()/5.0;
		...
		public function OnPaint(pDC:BitmapData, rcClient:Rectangle):void
		{
			...
			for (var i:uint = 0; i < m_CardDataArray.length; i++)
			{
				...
				if ((bDrawCard == true)&&(m_CardDataArray[i] != 0))
				{
					...
					m_ImageCard.DrawBitmapData(pDC,i*CARD_SPACE,0,
					GetCardXPos(bCardData),GetCardYPos(bCardData),
		...
		{
			if(bCardData % 13 == 0)
				return (uint(bCardData / 13) - 1) * CARD_HEIGHT;
			return uint(bCardData/13)*CARD_HEIGHT;
		}
