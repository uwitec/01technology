package org.god.Common.BetHistory.GameModule.B_GoldFlower
{
	...
	import org.god.GameModule.B_GoldFlower.Common.CMD_S_GameEnd;
	//游戏结束视图
	public class GameEndView extends BaseBattleGameEndView
	{
										...
										pIBetHistoryResource:IBetHistoryResource)
		{
			...
			SetWinScore(m_GameEnd.fGameGold,6);
			m_CardControl = new Array(6);
			var nXP:int = 88;
			var nYP:int = 56;
			...
			for(var i:int = 0; i < m_CardControl.length; i ++)
			{
				...
				m_CardControl[i].SetShowCardFlag(true);
				...
				{
					m_CardControl[i].SetBasicStation(new Point(nXP + 340, nYP + (i-3)* 92), 
												 enXCollocateMode.enXLeft,
												 enYCollocateMode.enYCenter);
				}
			}
