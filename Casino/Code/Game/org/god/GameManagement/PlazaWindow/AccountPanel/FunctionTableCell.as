package 
{
	...
	{
		public static const FunctionData:Array = [IDS_SETUSERSCORE,
												  IDS_BASICUSERINFO,
												  IDS_DETAILUSERINFO,
												  IDS_RIGHT,
												  IDS_OPERATION];
		...
		public function FunctionTableCell()
		{
			...
			if(m_arrFuncButton)
			{
				for(var i:uint = 0; i < m_arrFuncButton.length; i ++)
				{
					...
					if(FunctionData[i] != IDS_OPERATION)
					{
		...
		protected function AllowRightView():Boolean
		{
			...
			if(main)
			{
				if(main.getUserData())
				{
					return MasterRight.CanOperationView(main.getUserData().dwMasterRight);
				}
			}
			return false;
		}
