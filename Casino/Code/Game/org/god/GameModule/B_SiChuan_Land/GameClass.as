package 
{
	...
	import org.god.Net.*;
	import org.god.GameModule.B_SiChuan_Land.Common.*;
	...
	public class GameClass extends GameBaseClass
	{
		...
		private static  const IDI_DOUBLETIME:uint=205;//反牌定时器
		
		protected static  var TIME_OUT_CARD:uint=30;//出牌时间
		protected static  var TIME_MOST_OUT_CARD:uint=3;//叫牌时间
		
		protected static const nMaxPlayGameCount:uint = 1;//最大玩游戏次数
		...
		protected var m_wFirstOutUser:uint;//第一出牌玩家
		protected var m_cbLastTurnCardCount:uint;//查看上轮出牌次数
		protected var m_cbMoreTimeCount:uint;//延时次数
		protected var m_cbPlayGameCount:uint;//玩游戏次数
		...
		override protected function OnEventTimer(wChairID:uint,  nElapse:uint,  nTimerID:uint):Boolean
		{
			...
			switch (nTimerID)
			{
				...
				case IDI_DOUBLETIME://反牌
				{
					if (nElapse==0 || m_bStustee == true)
					{
						if ((IsLookonMode()==false)&&(wChairID==GetMeChairID()))
						{
							OnGiveUpDoubleTime(255,255);
						}
						return false;
					}
					if ((nElapse<=10)&&(wChairID==GetMeChairID())&&(IsLookonMode()==false))
		...
		override protected function OnGameMessage(wSubCmdID:uint, pBuffer:ByteArray, wDataSize:uint):Boolean
		{
			switch (wSubCmdID)
			{
				...
				case SUB_S_DOUBLETIME://反地主
				{
					return OnSubDoubleTime(pBuffer,wDataSize);
				}
				case SUB_S_USERDOUBLETIME://用户反牌
				{
					return OnSubUserDoubleTime(pBuffer,wDataSize);
				}
			};
		...
		override protected function OnGameSceneMessage(cbGameStation:uint,  bLookonOther:Boolean, pBuffer:ByteArray,  wDataSize:uint):Boolean
		{
			switch (cbGameStation)
			{
				...
				case GS_WK_PLAYING ://游戏状态
					{
						...
						for (var i:uint=0; i<GAME_PLAYER; i++)
						{
							...
							GetPeerGameView().SetUserDoubleTime(wViewChairID,pStatusPlay.bDoubleTimeScore[i]);
						}
						...
						if ((IsLookonMode()==false)&&(pStatusPlay.wCurrentUser==GetMeChairID()))
						{
							...
							GetPeerGameView().m_btDoubleTime.ShowWindow(false);
							GetPeerGameView().m_btDoubleTime.EnableWindow(false);
							GetPeerGameView().m_btGiveUpDoubleTime.ShowWindow(false);
							GetPeerGameView().m_btGiveUpDoubleTime.EnableWindow(false);
						}
				...
				case GS_WK_DOUBLETIME ://游戏状态
					{
						//效验数据
						if (wDataSize!=CMD_S_StatusDoubleTime.sizeof_CMD_S_StatusDoubleTime)
						{
							return false;
						}
						var pStatusDoubleTime:CMD_S_StatusDoubleTime=CMD_S_StatusDoubleTime.readData(pBuffer);

						//设置变量
						m_bTurnCardCount=0;
						m_bHandCardCount=pStatusDoubleTime.bCardCount[GetMeChairID()];
						CopyArray(m_bHandCardData,pStatusDoubleTime.bCardData,m_bHandCardCount);			
						
						//设置界面
						for (var i:uint=0; i<GAME_PLAYER; i++)
						{
							var wViewChairID:uint=SwitchViewChairID(i);
							m_bCardCount[i]=pStatusDoubleTime.bCardCount[i];
							GetPeerGameView().SetCardCount(wViewChairID,pStatusDoubleTime.bCardCount[i]);
							GetPeerGameView().SetUserDoubleTime(wViewChairID,pStatusDoubleTime.bDoubleTimeScore[i]);
						}
						GetPeerGameView().SetBombTime(pStatusDoubleTime.wBombTime);
						GetPeerGameView().SetBaseScore(pStatusDoubleTime.fBaseScore);
						GetPeerGameView().m_BackCardControl.SetCardData(pStatusDoubleTime.bBackCard,3);
						GetPeerGameView().m_HandCardControl.SetCardData(m_bHandCardData,m_bHandCardCount);
						GetPeerGameView().SetLandUser(SwitchViewChairID(pStatusDoubleTime.wLandUser),pStatusDoubleTime.bLandScore);
						var bCardData:Array = GenEmptyCardData(20);
						UpdateLeaveCardControl(bCardData);
						
						//玩家设置
						if ((IsLookonMode()==false)&&(pStatusDoubleTime.wCurrentUser==GetMeChairID()))
						{
							//GetPeerGameView().m_btOutCard.EnableWindow(false);
							...
							GetPeerGameView().m_btDoubleTime.ShowWindow(true);
							GetPeerGameView().m_btDoubleTime.EnableWindow(true);
							GetPeerGameView().m_btGiveUpDoubleTime.ShowWindow(true);
							GetPeerGameView().m_btGiveUpDoubleTime.EnableWindow(true);
						}

					

						//设置定时器
						SetGameTimer(pStatusDoubleTime.wCurrentUser,IDI_DOUBLETIME,30);
		...
		protected function OnSubSendCard(pBuffer:ByteArray,  wDataSize:uint):Boolean
		{
			...
			//设置时间
			if(pSendCard.wCurrentUser != INVALID_CHAIR)
		...
		//用户反牌
		protected function OnSubUserDoubleTime(pBuffer:ByteArray,  wDataSize:uint):Boolean
		{
			//效验数据
			if (wDataSize!=CMD_S_UserDoubleTime.sizeof_CMD_S_UserDoubleTime)
			{
				return false;
			}

			//消息处理
			var pUserDoubleTime=CMD_S_UserDoubleTime.readData(pBuffer);

			//设置界面
			var wViewChairID:uint=SwitchViewChairID(pUserDoubleTime.wDoubleTimeUser);
			GetPeerGameView().SetUserDoubleTime(wViewChairID,pUserDoubleTime.bDoubleTime);
			GetPeerGameView().SetUserDoubleView(wViewChairID,pUserDoubleTime.bDoubleTime);
			
			//玩家设置
			if ((IsLookonMode()==false)&&(pUserDoubleTime.wCurrentUser==GetMeChairID()))
			{
				ActiveGameFrame();
				GetPeerGameView().m_btDoubleTime.ShowWindow(true);
				GetPeerGameView().m_btDoubleTime.EnableWindow(true);
			...
			//设置时间
			SetGameTimer(pUserDoubleTime.wCurrentUser,IDI_DOUBLETIME,30);
		...
		protected function OnSubGameStart(pBuffer:ByteArray,  wDataSize:uint):Boolean
		{
			...
			//GetPeerGameView().SetUserDoubleTime(INVALID_CHAIR,0);
			GetPeerGameView().SetUserDoubleView(INVALID_CHAIR,0);
		...
		//反地主
		protected function OnSubDoubleTime(pBuffer:ByteArray,  wDataSize:uint):Boolean
		{
			//效验数据
			if (wDataSize!=CMD_S_DoubleTime.sizeof_CMD_S_DoubleTime)
			{
				return false;
			}

			//消息处理
			var pDoubleTime:CMD_S_DoubleTime=CMD_S_DoubleTime.readData(pBuffer);

			//设置变量
			m_wBombTime=1;
			m_bTurnCardCount=0;
			m_bTurnOutType=CT_INVALID;
			m_wLandUser=pDoubleTime.wLandUser;
			m_bCardCount[pDoubleTime.wLandUser]=20;
			ZeroArray(m_bTurnCardData);

			//设置控件
			GetPeerGameView().ShowLandTitle(false);
			GetPeerGameView().m_BackCardControl.SetCardData(pDoubleTime.bBackCard,pDoubleTime.bBackCard.length);

			//设置界面
			GetPeerGameView().SetLandScore(INVALID_CHAIR,0);
			GetPeerGameView().SetUserDoubleTime(INVALID_CHAIR,0);
			GetPeerGameView().SetUserDoubleView(INVALID_CHAIR,0);
			GetPeerGameView().SetCardCount(SwitchViewChairID(pDoubleTime.wLandUser),m_bCardCount[pDoubleTime.wLandUser]);

			//地主设置
			if (pDoubleTime.wLandUser==GetMeChairID())
			{
				var bCardCound:uint=m_bHandCardCount;
				m_bHandCardCount+=pDoubleTime.bBackCard.length;
				CopyArray(m_bHandCardData,
				pDoubleTime.bBackCard,
				pDoubleTime.bBackCard.length,
				bCardCound);
				m_GameLogic.SortCardList(m_bHandCardData,m_bHandCardCount);
				GetPeerGameView().m_HandCardControl.SetCardData(m_bHandCardData,m_bHandCardCount);
			}
			GetPeerGameView().SetLandUser(SwitchViewChairID(pDoubleTime.wLandUser),pDoubleTime.bLandScore);
			...
			//当前玩家
			if ((IsLookonMode()==false)&&(pDoubleTime.wCurrentUser==GetMeChairID()))
			{
				ActiveGameFrame();
				//GetPeerGameView().m_btOutCard.EnableWindow(false);
				GetPeerGameView().m_btOutCard.ShowWindow(false);
			...
			//设置时间
			SetGameTimer(pDoubleTime.wCurrentUser,IDI_DOUBLETIME,30);
		...
		protected function OnSubOutCard(pBuffer:ByteArray,  wDataSize:uint):Boolean
		{
			...
			GetPeerGameView().WarnRemainCard(wOutViewChairID,m_bCardCount[pOutCard.wOutCardUser]);
		...
		protected function OnSubGameEnd(pBuffer:ByteArray,  wDataSize:uint):Boolean
		{
			...
			if (IsLookonMode()==false)
			{
				if(m_cbPlayGameCount >= nMaxPlayGameCount)
				{
					GetPeerGameView().m_btStart.SetButtonImage(IMAGE_BT_StartAgain, 425,37);
				}
				GetPeerGameView().m_btStart.ShowWindow(true);
		...
		protected function OnGameViewEevent(e:GameViewEvent):void
		{
			switch(e.m_nMsg)
			{
				...
				case GameViewEvent.nGVM_DOUBLETIME:
				{
					 OnDoubleTime(e.m_nWParam,e.m_nLParam);
					 return;
				}
				case GameViewEvent.nGVM_GIVEUPDOUBLETIME:
				{
					 OnGiveUpDoubleTime(e.m_nWParam,e.m_nLParam);
					 return;
				}
				case GameViewEvent.nGVM_MORE_TIME:
				{
					 OnMoreTime(e.m_nWParam,e.m_nLParam);
					 return;
				}
			}
		}
		...
		//反牌按钮
		protected function OnDoubleTime(wParam:uint, lParam:uint):uint
		{
			//设置界面
			KillGameTimer(IDI_DOUBLETIME);
			GetPeerGameView().m_btDoubleTime.ShowWindow(false);
			...
			//发送数据
			var DoubleTime:CMD_C_DoubleTime = new CMD_C_DoubleTime;
			DoubleTime.bDoubleTime=2;
			var sendData:ByteArray =  DoubleTime.toByteArray();
			SendGameData(SUB_C_DOUBLETIME,
						 sendData,
						 CMD_C_DoubleTime.sizeof_CMD_C_DoubleTime);

			return 0;
		}
		//不反按钮
		protected function OnGiveUpDoubleTime(wParam:uint, lParam:uint):uint
		{
			//设置界面
			KillGameTimer(IDI_DOUBLETIME);
			GetPeerGameView().m_btDoubleTime.ShowWindow(false);
		...
		protected function OnOutCard(wParam:uint, lParam:uint):void
		{
			...
			{
				var bCardData:Array = new Array(20);
				if (GetPeerGameView().m_HandCardControl.GetShootCard(bCardData,bCardData.length)>0)
					GetPeerGameView().ShowBreakRuleView(true);
				return;
			}
			//状态判断
		...
		//延时
		protected function OnMoreTime(wParam:uint, lParam:uint):uint
		{
			//状态判断
			if (GetPeerGameView().m_btOutCard.IsWindowVisible()==false)
			{
			...
			//判断次数
			if(m_cbMoreTimeCount)
			{
				ShowMessageBox("每局游戏只能延时一次");
				return 0;
			}
			var wViewChairID:uint=this.SwitchViewChairID(GetMeChairID());
			var nElapse:uint = GetPeerGameView().GetUserTimer(wViewChairID);
			SetGameTimer(GetMeChairID(),IDI_OUT_CARD,nElapse + 30);
			m_cbMoreTimeCount ++;
			return 0;
		}
		//上轮扑克
		protected function OnLastTurnCard(wParam:uint, lParam:uint):uint
		{
			...
			if(m_cbLastTurnCardCount)
			{
				ShowMessageBox("每局游戏只能查看一次 上轮出牌");
				return 0;
			}
		...
		//判断上轮出牌情况
		protected function IsFirstOutCard():Boolean
		{
			for(var i:int = 0; i < 3; i ++)
			{
				if(m_cbOutCardCount[i][1])
					return false;
			}
