package 
{
	...
	public class CardControl extends Sprite
	{
		...
		//弹起扑克
		public function SetShootCard(cbCardData:Array, dwCardCount:uint):uint
		{
			//变量定义
			...
			for (var i:uint=0;i<dwCardCount;i++)
			{
				for (var j:uint=0;j<m_CardDataItem.length;j++)
				{
					var pCardItem:tagCardItem=m_CardDataItem[j];
		...
		{
			var nCardCount:int = int(dwCardCount);
			if(nCardCount < 0)
				return 0;
			//设置扑克
			SetCardDataItemSize(nCardCount);
			for (var i:uint=0;i<nCardCount;i++)
			{
				m_CardDataItem[i].bShoot=false;
